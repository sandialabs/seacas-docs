...
type:\quad{\sf SLAP}\quad and input the required data from the terminal. 
...
\KW{NOSE} or \KW{TAIL}, and then input the geometric quantities for
...
is terminated by inputting either \KW{EXIT} or \KW{END}.  A sample
...
value {\tt -9.999E0}.}\\[1ex] 
\KW{NOSE}  & --- & \parbox[t]{4in}{Indicates that the geometric
...
\KW{RADIUS} & \REQ & \parbox[t]{4in}{The distance from the
...
\KW{MU}     & 0.0 & \parbox[t]{4in}{Coefficient of Friction between body
and rigid surface at contact point}\\[1ex] 
\KW{SPRING} & \REQ & \parbox[t]{4in}{Indicates the beginning of the spring
...
\KW{END}}\\[1ex] 
\KW{UNLOAD} & 0.0 & \parbox[t]{4in}{Spring unloading modulus.  If this
...
{\tt-9.999E0}.}\\[1ex] 
\KW{MASS}   & \REQ & \parbox[t]{4in}{Body Mass}\\[1ex]
\KW{MOMENT} & \REQ & \parbox[t]{4in}{Mass Moment of Inertia about the
center of gravity. }\\[1ex] 
\KW{VERTICAL}& 0.0 & \parbox[t]{4in}{Initial vertical velocity, positive
upward.}\\[1ex] 
\KW{HORIZONTAL}& 0.0 & \parbox[t]{4in}{Initial horizontal velocity
(positive towards tail)}\\[1ex] 
\KW{THETA}   & 0.0 & \parbox[t]{4in}{ Initial angle in degrees.  Positive
counterclockwise if the nose is to the left of the tail.  Theta is
shown positive in Figure~\ref{f:geom}.}\\[1ex] 
\KW{OMEGA}   & 0.0 & \parbox[t]{4in}{ Initial angular velocity
(radians/time)}\\[1ex] 
\KW{GRAVITY} & 0.0 & \parbox[t]{4in}{Constant vertical acceleration
applied to body, positive upward. (length/time$^2$)}\\[1ex]
\KW{TIME}    & 0.1 & \parbox[t]{4in}{Time step scale factor applied to
the internally calculated time step.}\\[1ex] 
\KW{PLOT}    & 1   & \parbox[t]{4in}{The Plot data output interval
...
\KW{FRICTION}& 1.0 & \parbox[t]{4in}{The friction factor is used to
