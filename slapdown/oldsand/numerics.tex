\chapter{Description of Code Numerics}\label{theory}
...
\var{DISP}, (2)~the maximum spring displacement, \var{DISPMX} and (3)~a
...
simply searches the spring definition until the value of \var{DISP} is
...
and returns that value.  If the value of \var{DISP} is larger than the
...
value of the maximum displacement.  The variable \var{DELUL} is
calculated as the difference between the maximum displacement
(\var{DISPMX}) and the current displacement (\var{DISP}).  The force
corresponding to a displacement equal to \var{DISPMX} is calculated
using the method described in the previous paragraph.  This value is
then reduced by the quantity \var{DELUL}$\times$\var{UNLOAD}.  If the
